...
def nameSort():
    listsum = []
    for i in range(len(Names)):
        var1 = getData(Names, i)
        var2 = getData(Number, i)
        if var1 != None and var2 != None:
            listsum.append([var1, var2])
    listsum_new1 = sorted(listsum, key=operator.itemgetter(0))
    Names_new = []
    for item in listsum_new1:
...
class mainUI():
    def __init__(self):
        ...
        label.pack(side="top")
        label = Label(mainUI, text="한상진", font=('Helvetica', 9, "bold"), background = "#eee4da")
